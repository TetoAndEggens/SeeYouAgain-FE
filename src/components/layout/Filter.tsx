...
export default function Filter({ open, isOpen }: FilterProps) {
    ...
    return (
        open && (
            <div className="absolute z-10 flex flex-col items-center gap-5 rounded-b-md border border-gray-50 bg-white p-5 shadow-md">
                <div onClick={() => isOpen(false)} className="flex w-full justify-start gap-2">
                    <X size={20} /> 필터
                </div>
                ...
                <Button className="w-full px-5">확인</Button>
            </div>
        )
    );
}
